package com.hotel.RoomBooking.service.admin;
...
public class UserManagementService {
    ...
    public RequestResponse registerUser(RegisterDto registerDto){
        RequestResponse response = new RequestResponse();

        try{
            boolean existingEmail = userRepo.existsByEmail(registerDto.getEmail());
            if (!existingEmail){
                Users newUser = new Users();
                newUser.setName(registerDto.getName());
                newUser.setEmail(registerDto.getEmail());
                newUser.setPassword(passwordEncoder.encode(registerDto.getPassword()));
                newUser.setUserRole(registerDto.getUserRole());
                Users registeredUser = userRepo.save(newUser);
                if (registeredUser.getId()>0){
                    response.setMessage("Account Created successfully");
                    response.setStatusCode(201);
                    response.responseMapper(registeredUser);
                }
            }
            else {
                response.setStatusCode(403);
                response.setMessage("Failed to create user");
                response.setError("User with email exists "+ registerDto.getEmail());
            }
        } catch (Exception e) {
